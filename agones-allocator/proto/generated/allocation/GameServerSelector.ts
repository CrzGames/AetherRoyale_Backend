...
export const _allocation_GameServerSelector_GameServerState = {
  READY: 'READY',
  ALLOCATED: 'ALLOCATED',
} as const;

export type _allocation_GameServerSelector_GameServerState =
  | 'READY'
  | 0
  | 'ALLOCATED'
  | 1
...
export interface GameServerSelector {
  ...
  'counters'?: ({[key: string]: _allocation_CounterSelector});
  'lists'?: ({[key: string]: _allocation_ListSelector});
}
...
export interface GameServerSelector__Output {
  ...
  'counters': ({[key: string]: _allocation_CounterSelector__Output});
  'lists': ({[key: string]: _allocation_ListSelector__Output});
}
